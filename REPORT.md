...
├── run_full_dataset.py                 ← MAIN ENTRY POINT: generates all 4 damage cases × 50 runs
├── run_visualizations.py               ← Generates validation & modal analysis plots
├── requirements.txt / requirements_ml.txt  ← Python dependencies
├── REPORT.md                           ← This file
│
├── digital_twin/                       ← CORE SIMULATION PACKAGE
│   ├── __init__.py
│   ├── config.py                       ← All bridge parameters, VBI settings, damage definitions
│   ├── bridge_simulation.py            ← BridgeModel3D, SensorRecorder3D, VBI coupling, run_simulation()
│   ├── traffic.py                      ← Vehicle class, traffic generation, quarter-car parameters
│   └── utils.py                        ← Directory helpers, CSV writers
│
├── preprocessing/                      ← FEATURE EXTRACTION PIPELINE
│   ├── __init__.py
│   ├── features.py                     ← 42 statistical/FFT features per window + 42 cross-sensor spatial features
│   ├── run_feature_pipeline.py         ← Reads raw CSV → writes 3d_windowed_features.csv
│   └── window_dataset_cnn.py           ← Slices raw signals into per-sensor .npz window files for CNN training
│
├── models/                             ← ML TRAINING & EVALUATION
│   ├── __init__.py
│   ├── classifier_data_loader.py       ← Loads feature CSV, run-stratified split, scale, encode
│   ├── train_classifier.py             ← Trains RF/XGBoost/MLP/SVM on extracted features
│   ├── evaluate_classifier.py          ← Full evaluation: accuracy, F1, confusion matrices, ROC AUC
│   ├── train_cnn_binary.py             ← Model B: per-sensor binary 1D-CNN + intensity regression
│   ├── train_cnn_multiclass.py         ← Model A: per-sensor multiclass 1D-CNN
│   ├── train_gateway.py                ← RF gateway fusion: stacks CNN outputs → 4-class RF
│   ├── compare_models.py               ← Compares Model A vs Model B, prints winner
│   └── export_tflite.py                ← Exports binary CNN → TFLite for ESP32 deployment
│
├── validation/                         ← DATA EXPLORATION & VISUALIZATION
│   ├── __init__.py
│   ├── damage_analysis.py              ← PCA, spatial RMS patterns, damage indices, feature importance
│   └── fft_analysis.py                 ← FFT computation, spectral comparison, dominant frequency extraction
│
